	...
	list: async (req, res) => {
		...
		return res.json({
			meta: {
				...
				countByCategory: {
					comprador: users.filter(
						(user) => user.categoriausers_id == 1
					).length,
					vendedor: users.filter(
						(user) => user.categoriausers_id == 2
					).length,
					mecanico: users.filter(
						(user) => user.categoriausers_id == 3
					).length,
				},
			},
			data: users.map((user) => {
